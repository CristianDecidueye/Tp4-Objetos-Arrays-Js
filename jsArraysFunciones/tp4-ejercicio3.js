...
const biblioteca = {
  ...
  buscarLibroPorTitulo: function (titulo) {
    const libroEncontrado = this.catalogoLibros.find(
    ...
    if (libroEncontrado) {
      ...
      document.write(
        `El libro "${titulo}" no se encuentra en el catálogo.<br>`
      );
    }
  },
  ...
  verificarDisponibilidad: function (titulo) {
    ...
    if (libroEncontrado) {
      if (libroEncontrado.disponible) {
        document.write(
          `El libro "${titulo}" está disponible para prestarlo.<br>`
        );
      } else {
        document.write(
          `El libro "${titulo}" no está disponible para prestarlo.<br>`
        );
      }
    } else {
      document.write(
